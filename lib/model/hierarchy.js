 ...
 * @returns
 */
export function allocateDepth(results, connects, nodesOfCurrentDepth, depth) {
  const deeperNodes = [];
 ...
 * @returns
 */
export function locateCoordinate(results, connects, gap) {
  const [w, h] = gap;
  let minx = 0;
  let maxX = 0;
  results.forEach(({ depth, nodes }) => {
    const xOfSameDepth = nodes.reduce(
      ...
      (xs, node) => {
        ...
        const parentsX = Array.from(connects[node.id] || []).reduce(
